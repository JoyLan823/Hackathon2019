...
def maneuver(lat, long, maneuver):
    #If user's current lat and long is close enough to lat and long for next
...
data = requests.get(url) #Stores entire JSON object fetched from the REST endpoint
json_data = data.json()
